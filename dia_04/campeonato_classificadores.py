...
MODELO_FLASH = os.getenv("GEMINI_MODEL", "gemini-3.8-flash")
...
BAIXA: estética, sugestão ou feedback sem impacto operacional.
MEDIA: degradação parcial, problema individual ou alternativa disponível.
ALTA: indisponibilidade crítica, perda financeira ou incidente de segurança.
...
Urgencia:"""
    ...
    saida = (response.text or "").strip().upper()
    return saida if saida in {"BAIXA", "MEDIA", "ALTA"} else "DESCONHECIDO"
...
"""
    ...
    saida = (response.text or "").strip()
    resultado = re.search(r"(?:^|\n)Urg[eê]ncia:\s*(BAIXA|MEDIA|ALTA)\s*$", saida, re.IGNORECASE)
    return resultado.group(1).upper() if resultado else "DESCONHECIDO"
